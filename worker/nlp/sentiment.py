...
BULL_TERMS = {
    ...
    "squeeze": 1.8,
    "gamma ramp": 2.0,
    "shorts are trapped": 2.8,
    "short interest is fuel": 2.5,
    "executing flawlessly": 2.5,
    "has legs": 2.0,
    "next leg up": 2.2,
    "thesis keeps getting stronger": 2.5,
    "up only": 2.0,
    "outperform": 1.8,
    "upgrade": 1.8,
    "upgraded": 1.8,
    "demand is bottomless": 2.5,
    "backlog at record": 2.2,
    "paying off": 1.8,
    "bullish continuation": 2.5,
    "reclaimed": 1.5,
    "best risk/reward": 2.2,
    "decoupling is real": 1.5,
    "institutions are here": 1.8,
    "inflows": 1.5,
    "leading the charge": 1.8,
    "positioned perfectly": 2.2,
    "winner": 1.5,
    "crushing it": 2.2,
    "underrated": 1.5,
    "quietly at all-time highs": 2.2,
}
BEAR_TERMS = {
    "puts": 1.2,
    "puts printing": 3.0,
    ...
    "dump": 1.8,
    "dumping": 2.0,
    "insiders dumping": 2.8,
    "sell off": 2.0,
    "selloff": 2.0,
    "overvalued": 2.2,
    "priced for perfection": 2.5,
    "makes zero sense": 2.0,
    "bagholder": 2.5,
    "bag holder": 2.5,
    "wait for the flush": 2.2,
    "margins are getting crushed": 2.8,
    "margin death spiral": 3.0,
    "guidance was weak": 2.5,
    ...
    "taking profits": 1.5,
    "sold all": 2.0,
    "momentum is gone": 2.2,
    "in trouble": 2.0,
    "not good": 1.8,
    "numbers don't add up": 2.5,
    "don't add up": 2.2,
    "giving back": 1.8,
    "underwhelmed": 2.0,
    "bulls in denial": 2.5,
...
NEGATORS = re.compile(
    r"\b(not|no|never|isn't|aren't|won't|don't|doesn't|wasn't)\s+(\w+\s+){0,2}$"
)
INTENSIFIERS = {
    "absolutely": 1.4,
    "insanely": 1.4,
...
def score_finbert_local(posts: list[Post], batch_size: int = 32) -> None:
    ...
    clf = pipeline(
        "text-classification", model=FINBERT_MODEL, top_k=None, truncation=True
    )
...
def score_hf_api(posts: list[Post], batch_size: int = 50) -> None:
    ...
    for i in range(0, len(posts), batch_size):
        ...
        resp = requests.post(
            ...
            headers=headers,
            json={
                "inputs": [p.text[:512] for p in chunk],
