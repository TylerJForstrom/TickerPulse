...
def embed_and_project(texts: list[str], backend: str) -> np.ndarray:
    ...
    return TSNE(
        n_components=2, perplexity=perplexity, random_state=42, init="pca", max_iter=600
    ).fit_transform(Xr)
...
def ctfidf_labels(
    ...
    for c, counts in tf.items():
        ...
        scores = {
            ...
            term: (cnt / total)
            * np.log(1 + n_clusters / df[term])
            * (1.6 if " " in term else 1.0)
        ...
        for p in members:
            if p.timestamp >= series_start:
                idx = min(
                    n_series - 1,
                    int(
                        (p.timestamp - series_start).total_seconds()
                        // (SERIES_BUCKET_H * 3600)
                    ),
                )
                series[idx] += 1
        half = n_series // 2
        ...
        topics.append(
            {
                ...
                "tickers": [
                    {"ticker": t, "count": n} for t, n in tickers.most_common(6)
                ],
                "series": series,
                "trend": "rising"
                if late > early * 1.3
