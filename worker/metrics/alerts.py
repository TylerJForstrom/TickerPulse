    ...
    for t, m in trends.items():
            ...
            alerts.append(
                {
                    "ticker": t,
                    "kind": "mention_spike",
                    "score": m["breakout_score"],
                    "message": (
                        f"${t} mention volume is {m['breakout_score']:.1f}σ above its trailing "
        ...
        if m["mentions_prev"] >= 5 and m["mentions"] >= 5:
            ...
            if m["sentiment_avg"] <= -0.25 and m["bull_bear_ratio"] < 0.6:
                alerts.append(
                    {
                        "ticker": t,
                        "kind": "bearish_turn",
                        "score": abs(m["sentiment_avg"]),
                        "message": (
                            f"${t} sentiment has turned decisively bearish "
                ...
                m["sentiment_avg"] >= 0.35
                and m["bull_bear_ratio"] > 3.0
                and m["breakout_score"] > 1.0
            ):
                alerts.append(
                    {
                        "ticker": t,
                        "kind": "bullish_surge",
                        "score": m["sentiment_avg"],
                        "message": (
                            f"${t} is seeing a bullish surge "
